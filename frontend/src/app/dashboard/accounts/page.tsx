    ...
    return (
        <>
            <div className="flex justify-between items-center mb-6">
                ...
                <button className="bg-white border border-white/10 text-white hover:bg-black/20 font-bold px-4 py-2 rounded-lg text-sm shadow-sm transition flex items-center gap-2">
                    <Download size={16} /> Export Ledgers
                </button>
            </div>

            <div className="liquid-glass-card rounded-xl    min-h-[500px] flex flex-col">
                {/* Tabs */}
                <div className="flex border-b border-white/10 bg-black/20 rounded-t-xl">
                    <button
                        onClick={() => setActiveTab('overview')}
                        className={`flex-1 py-4 font-bold text-sm transition-colors border-b-2 ${activeTab === 'overview' ? 'border-indigo-600 text-indigo-700 bg-white' : 'border-transparent text-glass-body hover:text-glass-title'}`}
                    >
                        <div className="flex justify-center items-center gap-2"><BarChart3 size={16} /> Profit & Loss Overview</div>
                    </button>
                    <button
                        onClick={() => setActiveTab('expenses')}
                        className={`flex-1 py-4 font-bold text-sm transition-colors border-b-2 ${activeTab === 'expenses' ? 'border-indigo-600 text-indigo-700 bg-white' : 'border-transparent text-glass-body hover:text-glass-title'}`}
                    >
                        <div className="flex justify-center items-center gap-2"><Receipt size={16} /> Expense Vouchers</div>
                    </button>
                    <button
                        onClick={() => setActiveTab('compliance')}
                        className={`flex-1 py-4 font-bold text-sm transition-colors border-b-2 ${activeTab === 'compliance' ? 'border-indigo-600 text-indigo-700 bg-white' : 'border-transparent text-glass-body hover:text-glass-title'}`}
                    >
                ...
                <div className="p-6 flex-1">
                        ...
                        <div className="space-y-6">
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                <div className="bg-black/20 p-6 rounded-xl border border-white/10">
                                    <div className="text-sm font-bold text-glass-body mb-1 flex items-center gap-2">Gross Revenue (Total Sales)</div>
                                    <div className="text-3xl font-bold text-glass-title">₹{summary.summary.totalRevenue.toLocaleString()}</div>
                                    <div className="text-xs text-glass-body mt-2">Aggregate of all paid patient bills (OPD, IPD, Lab, Pharma).</div>
                                </div>
                                <div className="bg-black/20 p-6 rounded-xl border border-white/10">
                                    <div className="text-sm font-bold text-glass-body mb-1 flex items-center gap-2">Total Expenses (Outflow)</div>
                                    <div className="text-3xl font-bold text-glass-title">₹{summary.summary.totalExpense.toLocaleString()}</div>
                                    <div className="text-xs text-glass-body mt-2">Aggregate of all paid vendor and operational expenses.</div>
                                </div>
                                <div className={`p-6 rounded-xl border ${summary.summary.netProfit >= 0 ? 'bg-emerald-50 border-emerald-200' : 'bg-rose-50 border-rose-200'}`}>
                                    <div className="text-sm font-bold text-glass-muted mb-1 flex items-center gap-2"><TrendingUp size={16} /> Net Profit Tracker</div>
                                    ...
                                    <div className="text-xs text-glass-muted mt-2 font-medium">Real-time P&L status across all integrated models.</div>
                                </div>
                            </div>

                            <h3 className="font-bold text-glass-title text-lg border-b pb-2 mt-8">Registered Fixed & Current Ledgers</h3>
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                {summary.ledgers.map((l: any) => (
                                    <div key={l.id} className="border border-white/10 p-4 rounded-xl shadow-sm">
                                        <div className="text-xs font-bold text-slate-400 uppercase tracking-widest">{l.group}</div>
                                        <div className="font-bold text-glass-title mt-1">{l.name}</div>
                                    </div>
                                ))}
                                {summary.ledgers.length === 0 && <div className="text-glass-body text-sm italic col-span-4 p-4 text-center bg-black/20 border rounded-xl">No manual ledgers created yet. Operating on logical aggregates.</div>}
                            </div>
                        </div>
                    )}
                    ...
                    {activeTab === 'expenses' && (
                        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                            {/* New Expense Form */}
                            <div className="lg:col-span-4 space-y-4 bg-black/20 p-6 rounded-xl border border-white/10 h-fit">
                                <h3 className="font-bold text-glass-title pb-2 border-b">Create Voucher / Expense</h3>
                                <form onSubmit={handleAddExpense} className="space-y-3">
                                    <div>
                                        <label className="block text-xs font-semibold mb-1 text-glass-muted">Expense Category</label>
                                    ...
                                    <div>
                                        <label className="block text-xs font-semibold mb-1 text-glass-muted">Description / Payee Name</label>
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        <div>
                                            <label className="block text-xs font-semibold mb-1 text-glass-muted">Date</label>
                                        ...
                                        <div>
                                            <label className="block text-xs font-semibold mb-1 text-glass-muted">Mode</label>
                                    ...
                                    <div className="bg-white p-3 rounded-lg border border-white/10 mt-2">
                                        <div>
                                            <label className="block text-xs font-bold mb-1 text-white">Base Amount (₹)</label>
                                        ...
                                        <div className="grid grid-cols-2 gap-2 mt-2">
                                            <div>
                                                <label className="block text-[10px] uppercase font-bold text-glass-body mb-1">Input GST (+)</label>
                                            ...
                                            <div>
                                                <label className="block text-[10px] uppercase font-bold text-glass-body mb-1">TDS Deduct (-)</label>
                                        ...
                                        <div className="text-xs text-right mt-3 text-glass-body font-bold">
                                            Total Payout: <span className="text-base text-glass-title ml-1">₹{(Number(amount || 0) + Number(gstAmount || 0) - Number(tdsAmount || 0)).toFixed(2)}</span>
                                        </div>
                                    </div>

                                    <button disabled={loading} type="submit" className="w-full liquid-glass-button text-white font-bold py-3 mt-4 rounded-lg shadow transition">
                                        Record Voucher
                                    </button>
                                </form>
                            </div>
                            ...
                            <div className="lg:col-span-8">
                                <h3 className="font-bold text-glass-title pb-2 border-b mb-4">Voucher Ledger</h3>
                                <div className="overflow-x-auto">
                                    <table className="w-full text-left text-sm bg-white border border-white/10 rounded-xl">
                                        <thead className="bg-slate-100 text-glass-muted font-semibold border-b border-white/10">
                                            <tr>
                                        ...
                                        <tbody className="divide-y divide-white/5">
                                            {expenses.map(e => (
                                                <tr key={e.id} className="hover:bg-black/20">
                                                    <td className="p-3 font-mono text-xs text-glass-body">{e.voucherNo}</td>
                                                    <td className="p-3">
                                                        <div className="font-bold text-glass-title">{e.category}</div>
                                                        <div className="text-xs text-glass-body">{e.description} • {e.paymentMode}</div>
                                                    </td>
                                                    <td className="p-3 text-xs">
                                                        ...
                                                        <div className="text-sm font-bold mt-1 text-glass-title border-t pt-1">Net: ₹{e.netAmount}</div>
                                                    </td>
                    ...
                    {activeTab === 'compliance' && summary && (
                        <div className="max-w-4xl mx-auto space-y-6">
                            ...
                            <div className="bg-indigo-50 border border-indigo-200 p-6 rounded-xl flex items-center justify-between">
                                ...
                                <button className="liquid-glass-button text-white font-bold px-5 py-2 rounded-lg shadow transition">Export GSTR Format</button>
                            </div>

                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                {/* Outward */}
                                <div className="border border-white/10 rounded-xl p-6 bg-white shadow-sm">
                                    <div className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-2">Total Outward GST Collected</div>
                                    <div className="text-3xl font-bold text-glass-title">₹{summary.compliance.outwardGstCollected.toLocaleString()}</div>
                                    <div className="text-xs text-glass-body mt-2 font-medium">Billed to patients on IPD, OPD, LAB bills. Liability to pay to gov.</div>
                                </div>
                                ...
                                <div className="border border-white/10 rounded-xl p-6 bg-white shadow-sm">
                                    ...
                                    <div className="text-xs text-glass-body mt-2 font-medium">Paid to registered vendors. Deductible from liability.</div>
                                </div>
